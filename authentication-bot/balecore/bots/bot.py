...
class Bot:
    ...
    async def start_polling(self):
        if not self.token or not self.token.strip():
            logger.error("Token is empty! Please provide a valid bot token.")
            return
        ...
        try:
            ...
            if bot_info and hasattr(bot_info, 'username') and bot_info.username:
                logger.info(f"Service started | Client: @{bot_info.username} (ID: {bot_info.id})")
            else:
                logger.error("Invalid token! Please check your token and provide a valid one.")
                self.running.clear()
                return

        except Exception as e:
            logger.error(f"An error occurred while starting the bot: {e}")
            logger.error("Please check your token and API URL.")
            self.running.clear()
            return
